 ...
 *
 * Persistence: the fix-list is saved to localStorage, so captures SURVIVE page
 * navigation. Each item is tagged with the page it was captured on, and
 * "複製全部" groups the output by page so cross-page edits stay unambiguous.
 *
 * Collapse: the panel header has a 收合 button. Collapsed, it shrinks to a small
 * pill on the right edge (showing the item count) so the page behind it is fully
 * visible; click the pill to expand again. The collapsed state is remembered too.
 *
...
(function () {
  'use strict';
  var items = [], panel = null, list = null, btn = null, reopen = null, collapsed = false;
  var LS_ITEMS = '__dbg_overlay_items_v1';
  var LS_COLLAPSED = '__dbg_overlay_collapsed_v1';
  ...
  // ---------- persistence ----------
  // Identify the current page so cross-page captures stay distinguishable.
  function pageId() {
    return (location.pathname.split('/').pop() || location.pathname || 'index') + (location.hash || '');
  }
  function save() {
  ...
  function load() {
    try {
      ...
      if (raw) { var a = JSON.parse(raw); if (Array.isArray(a)) items = a; }
  ...
  function add(it) {
    it.page = pageId();
    // dedupe within the same page (same loc + selected text)
    if (items.some(function (x) { return x.loc === it.loc && x.sel === it.sel && x.page === it.page; })) return;
    items.push(it);
    save();
    render();
  }
  ...
  function updateCounts() {
    var n = items.length;
    var c = panel && panel.querySelector('#dbg-count');
    if (c) c.textContent = n;
    var rc = reopen && reopen.querySelector('.dbg-rc');
  ...
  function render() {
    ...
    list.innerHTML = items.length ? items.map(function (it, k) {
      ...
      // page tag — dim if the item belongs to a different page than the one we're on
      var pageTag = it.page
        ? '<span class="dbg-page' + (it.page === here ? '' : ' other') + '">' + esc(it.page) + '</span>' : '';
      ...
      return '<div class="dbg-row">' +
        ...
        '<div class="dbg-loc">' + pageTag + badge + esc(it.loc) + '</div>' +
        '<div class="dbg-cur">選取: <b>' + esc(it.sel) + '</b></div>' +
        '<div class="dbg-chips">' + chips + '</div>' + fields +
        '</div>';
    }).join('') : '<div class="dbg-empty">圈選頁面文字加入清單,或點擊單一文字元件。清單會跨分頁保留。</div>';
    updateCounts();
  }

  function copyAll() {
    // group items by the page they were captured on, so the paste-back is unambiguous
    var order = [], groups = {};
    items.forEach(function (it) {
      var pg = it.page || '(unknown)';
      if (!groups[pg]) { groups[pg] = []; order.push(pg); }
      groups[pg].push(it);
    });
    var text = order.map(function (pg) {
      var body = groups[pg].map(function (it) {
  ...
  function applyPanelVis() {
    if (!panel) return;
    if (!active()) { panel.style.display = 'none'; if (reopen) reopen.style.display = 'none'; return; }
  ...
  function buildPanel() {
    ...
    panel.innerHTML =
      '<div class="dbg-h">' +
      '<span class="dbg-title">🐞 修正清單</span>' +
      '<span id="dbg-count">0</span>' +
      '<button id="dbg-collapse" title="收合面板">收合 ›</button>' +
      '<button id="dbg-clear">清空</button>' +
      '<button id="dbg-copy">複製全部</button></div>' +
      '<div id="dbg-list"></div>';
    document.body.appendChild(panel);

    // collapsed-state pill on the right edge — click to expand
    reopen = document.createElement('button');
    reopen.id = 'dbg-reopen';
    reopen.type = 'button';
    reopen.title = '展開修正清單';
    reopen.innerHTML = '🐞<span class="dbg-rc">0</span>';
    ...
    list.addEventListener('input', function (e) {
      ...
      if (a) { a.val = e.target.value; save(); }
    });
    list.addEventListener('click', function (e) {
      var t = e.target;
      if (t.classList.contains('dbg-del')) { items.splice(+t.dataset.k, 1); save(); render(); return; }
    ...
    panel.querySelector('#dbg-collapse').onclick = function () { setCollapsed(true); };
    panel.querySelector('#dbg-clear').onclick = function () { items.length = 0; save(); render(); };
  ...
  function moveHl(e) {
    if (!active() || !hl) return;
    var t = e.target;
    if (t === btn || t === hl || t === reopen ||
        (t.closest && (t.closest('#dbg-panel') || t.closest('#dbg-reopen')))) { hl.style.display = 'none'; return; }
  ...
  document.addEventListener('mouseup', function (e) {
    if (!active()) return;
    if ((e.target.closest && (e.target.closest('#dbg-panel') || e.target.closest('#dbg-reopen'))) || e.target === btn) return;
    setTimeout(function () {
